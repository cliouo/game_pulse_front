...
const formatPrice = (value?: number, currency = "CNY") => {
  ...
  if (symbol) return `${symbol}${priceFormatter.format(value)}`
  ...
  return (
    <div className="space-y-6">
      <div className="space-y-1">
        <h1 className="text-2xl font-bold">SteamDB 工具</h1>
        {subtitleLoading ? (
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
        ...
        <TabsContent value="sales" className="space-y-4">
                    ...
                    : salesRecords.length > 0
                      ? salesRecords.map((record) => (
                          <TableRow key={`sales-${record.app_id}-${record.detected_at}`}>
                            ...
                            <TableCell className="text-muted-foreground line-through">{formatPrice(record.original_price, record.currency)}</TableCell>
                            <TableCell className="font-semibold text-emerald-600">{formatPrice(record.sale_price, record.currency)}</TableCell>
                            <TableCell>
                              <Badge variant="destructive" className="min-w-14 justify-center">-{Math.round(record.discount_percent)}%</Badge>
                            </TableCell>
                          </TableRow>
                        ))
                      : (
          ...
          <Pagination
            ...
            onPageSizeChange={(v) => { setSalesPageSize(v); setSalesPage(1) }}
          />
        </TabsContent>
              ...
              {searchAppId === null ? (
                ...
                <div className="flex flex-wrap gap-2">
                  {Array.from({ length: 12 }).map((_, index) => (
                    <Skeleton key={`tag-sk-${index}`} className="h-8 w-24" />
                  ))}
                </div>
              ) : tagsQuery.isError ? (
